package de.ar.tools.codenameone.encryption.md5;
...
public class MD5 {
	...
	private byte[] Encode(int input[], int len) {
		...
		for (i = j = 0; j < len; i++, j += 4) {
			byte o1 = (byte) (input[i] & 0xff);
			byte o2 = (byte) ((input[i] >>> 8) & 0xff);
			byte o3 = (byte) ((input[i] >>> 16) & 0xff);
			byte o4 = (byte) ((input[i] >>> 24) & 0xff);
	...
	public synchronized byte[] Final() {
		...
		byte[] encode = Encode(finals.state, 16);
		return encode;
	}
